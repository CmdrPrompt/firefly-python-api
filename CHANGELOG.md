...
- `FireflyClient.get_transactions_by_type(transaction_type, start, end, on_page=None)` fetches transactions of one or more comma-separated types (e.g. `"withdrawal,deposit"`) in a date range via the global `GET /api/v1/transactions?type=...` endpoint, which filters correctly (unlike the per-account endpoint, see below); `get_withdrawal_transactions()` is now implemented on top of it with no change to its own behavior. `TransactionRead` records also gain `destination_id`, letting consumers identify deposit transactions belonging to an account (TASK-016)
- Integration test coverage now spans every read-only `FireflyClient` method against a real Firefly III instance, closing the previous gap for `get_transactions_for_account()` and `get_budget_limits()` (TASK-017)
...
- `get_latest_transaction_date(account_id)`'s TASK-015 `transaction_type` filter was reverted: verified against a real Firefly III instance, the per-account transactions endpoint ignores the `type` query parameter entirely. Consumers needing a type-filtered latest date should use `get_transactions_by_type()` instead (TASK-016)
